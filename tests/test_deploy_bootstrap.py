...
def test_matching_version_needs_no_bootstrap(monkeypatch):
    monkeypatch.setattr(bootstrap, "_installed_version", lambda: "0.10.3")
    monkeypatch.setattr(
...
def test_different_patch_version_bootstraps(monkeypatch, tmp_path):
    ...
    deploy_python = tmp_path / "envs-xmpp" / "deploy" / "0.10.3" / "bin" / "python"
    assert calls[0] == [
    ...
    assert calls[1][-1] == "envs-xmpp==0.10.3"
