...
`!room rejoin <room|all>`, startup joins, `!sync`, and health-check recovery all use the same tracked join path and `MUC_JOIN_TIMEOUT_SECONDS`. Startup and explicit join operations also use `MUC_JOIN_RETRIES`; health recovery deliberately overrides it with one attempt per cycle because the worker already provides its own bounded 60/120/240/300-second retry schedule. It remains at 300 seconds until recovery, then returns to `HEALTH_CHECK_INTERVAL`. Successful administrative recovery also reapplies active bans to the room.
